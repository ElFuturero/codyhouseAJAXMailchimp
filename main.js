jQuery(document).ready(function($){
	...
	//function to submit MailChimp form
	...
	function register($form) {
	  $.ajax({
	    ...
	    error       : function(err) { 
	    	...
	    	$('#response-error').html('Could not connect to server. Please try again later');
	    	 },
	    success     : function(data) {
	      ...
	      if (data.result != "success") {
	        ...
	        $('#response-notification').html('<p>'+message+'</p>');
	      } 
	 
	      else {
	        var message = data.msg;
	        $('.cd-response-success').addClass('slide-in');
	        $('#response-success').html('<p>'+message+'</p>');
	      }
	    }
	  });
	}
